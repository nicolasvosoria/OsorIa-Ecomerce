...
import { POST } from "@/app/api/admin/shop-config/route";
import {
  ACTIVE_STORE_COOKIE,
  signActiveStore,
...
function makeExistingConfigQuery(existingId?: string) {
  return {
...
function makeRequest(body?: unknown, headers: Record<string, string> = {}) {
  return new NextRequest("http://localhost/api/admin/shop-config", {
...
describe("shop config admin route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects a payload whose config is not an object", async () => {
    const response = await POST(makeRequest({ config: "not-an-object" }));

    expect(response.status).toBe(400);
    expect(createClient).not.toHaveBeenCalled();
  });

  it("rejects non-admin users before writing the config", async () => {
    const serviceSchema = {
    ...
    const response = await POST(
      makeRequest({ config: { defaultSort: "newest" } }),
    );

    expect(response.status).toBe(403);
    expect(serviceSchema.from).not.toHaveBeenCalledWith("shop_config");
  });

  it("sanitizes and upserts the config for authenticated admins", async () => {
    const existingConfigQuery = makeExistingConfigQuery("config-1");
    const updatedRow = {
      id: "config-1",
      store_id: "store-1",
      config: {
    ...
    const configTable = {
      select: vi.fn().mockReturnValue(existingConfigQuery),
    ...
    const serviceSchema = {
      ...
      from: vi.fn((table: string) => {
        if (table === "stores_legacy") return makeStoresLegacyQuery("store-1");
        if (table === "shop_config") return configTable;
        throw new Error(`unexpected table ${table}`);
      }),
    };
    ...
    const response = await POST(
      makeRequest({
        config: {
          defaultSort: "bogus-sort",
          filters: { category: false, bogusFilter: true },
        },
      }),
    );

    expect(response.status).toBe(200);
    expect(configTable.update).toHaveBeenCalledWith(
    ...
    await expect(response.json()).resolves.toEqual({ data: updatedRow.config });
  });
  ...
  it("writes to the signed active store instead of the host store", async () => {
    const updateChain = {
      ...
      single: vi.fn().mockResolvedValue({
        data: { id: "config-2", config: {} },
        error: null,
      }),
    };
    const configTable = {
      select: vi.fn().mockReturnValue(makeExistingConfigQuery("config-2")),
      update: vi.fn().mockReturnValue(updateChain),
    };
    const serviceSchema = {
      rpc: makeCanManageStoreRpc(true),
