...
import { getStoreIdSync } from "@/lib/utils/store"
...
export function StylesProvider({ children }: { children: ReactNode }) {
  ...
  const getCurrentStoreId = (): string | null => {
    ...
    // Usar getStoreIdSync que obtiene de cookies/localStorage
    return getStoreIdSync()
  }
  ...
  const refreshStyles = async () => {
    try {
      ...
      try {
        ...
        if (cachedStyles) {
          const cachedData = JSON.parse(cachedStyles)
          const cachedMap = new Map(Object.entries(cachedData))
          setStyles(cachedMap)
          console.log(`[v0] Loaded cached styles from localStorage for store ${currentStoreId}:`, cachedMap.size, "components")
        }
      } catch (e) {
      ...
      if (stylesMap.size > 0) {
        console.log(`[v0] Loaded styles from Supabase for store ${currentStoreId}:`, stylesMap.size, "components")
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const attemptLoadStyles = () => {
        const currentStoreId = getCurrentStoreId()
        if (currentStoreId) {
          console.log(`[v0] StoreId disponible: ${currentStoreId}, cargando estilos...`)
          refreshStyles()
          return true
        }
        return false
        ...
        const interval = setInterval(() => {
          ...
          if (attemptLoadStyles() || attempts >= maxAttempts) {
            ...
            if (attempts >= maxAttempts) {
              console.warn("[v0] No se pudo obtener storeId después de varios intentos")
              setLoading(false)
            }
          }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const handleStorageChange = (e: StorageEvent) => {
        if (e.key === 'osoria_current_store_id' || e.key?.startsWith('osoria_component_styles_')) {
          console.log('[v0] Cambio detectado en localStorage, recargando estilos...')
          refreshStyles()
        }
      }
      ...
      const checkInterval = setInterval(() => {
        ...
        if (currentStoreId && currentStoreId !== lastStoreId) {
          console.log(`[v0] StoreId cambió de ${lastStoreId} a ${currentStoreId}, recargando estilos...`)
  ...
  useEffect(() => {
    ...
    try {
      channel = subscribeToStyleChanges(componentName, (newStyle) => {
        ...
        try {
          const currentStoreId = getStoreIdSync()
          if (currentStoreId) {
            const storageKey = `osoria_component_styles_${currentStoreId}`
